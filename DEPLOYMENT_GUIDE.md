...
| `accounting` | `UserRoleEnum::ACCOUNTING` | `accounting.dashboard` | Financial ops, student archive read. Admin role also has access to accounting routes. |
...
`app/Enums/AdminTypeEnum.php` has been deleted. Do not reference or recreate it.
...
│   ├── Dashboard.vue
│   ├── Analytics.vue       ← stub/placeholder
│   ├── Index.vue
│   ├── Reports.vue         ← stub/placeholder
│   └── Show.vue
...
├── Notifications/
│   └── Index.vue           ← student-facing notifications (separate from Admin/Notifications/)
└── settings/
    ├── Profile.vue
    ├── Password.vue
    └── Appearance.vue
...
>
> ⚠️ **Dead page directories:** `pages/Fees/` and `pages/Subjects/` contain Vue pages for the Fee Management and Subject Management features, which are **disabled** (routes are commented out in `routes/web.php`). Do not route to or render these pages. They are candidates for deletion.
...
- `faculty`, `status`, `department`, `is_active`, `email_verified_at`
...
accounting.dashboard               → GET    /accounting/dashboard  ← accessible by accounting AND admin
accounting.transactions.index      → GET    /accounting/transactions
users.{index|create|show|edit|...} → /admin/users/*
notifications.{index|create|...}   → /admin/notifications/*
notifications.dismiss              → POST   /notifications/{notification}/dismiss  ← auth-only, NOT admin-only
...
students.reinstate                 → POST   /students/{student}/reinstate  ← admin only
...
student-fees.drop                  → POST   /student-fees/{user}/drop  ← admin+accounting
...
accounting-workflows.index         → GET    /accounting-workflows
accounting-workflows.create        → GET    /accounting-workflows/create
accounting-workflows.store         → POST   /accounting-workflows
accounting-workflows.show          → GET    /accounting-workflows/{transaction}
accounting-workflows.update        → PUT    /accounting-workflows/{transaction}
accounting-workflows.destroy       → DELETE /accounting-workflows/{transaction}
accounting-workflows.submit        → POST   /accounting-workflows/{transaction}/submit
...
**ORM:** Eloquent with SoftDeletes on `Student` model only. `User` model does NOT use SoftDeletes — user deletions are hard deletes.

### Key Models & Relationships

```
User  (no SoftDeletes — hard delete on destroy)
...
| Assuming `User` model uses SoftDeletes | It does NOT — only `Student` uses SoftDeletes. User deletes are permanent. |
| Referencing or recreating `AdminTypeEnum` | It has been deleted — admin sub-types no longer exist |
| Using `route('my-profile')` or `route('claude.guide')` | These routes do not exist — remove any references |
| Assuming `accounting.dashboard` is accounting-only | Admin role also has access (`role:accounting,admin` guard) |
| Assuming `notifications.dismiss` is admin-only | It is `auth`-only — all authenticated roles can call it |
